...
interface StyleMatchesSectionProps {
  profiles: DiscoverProfile[];
  loading: boolean;
  onProfilePress: (profile: DiscoverProfile) => void;
}

export default function StyleMatchesSection({
  profiles,
  loading,
  onProfilePress,
}: StyleMatchesSectionProps) {
  if (loading) {
    return (
      <View style={styles.container}>
        <View style={styles.header}>
          <Text style={styles.title}>Style Matches</Text>
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.subtitle}>Based on your wardrobe & style preferences</Text>
      <FlatList
        data={profiles}
        horizontal
