...
const ProfileScreen = () => {
  ...
  const fetchProfileData = async (userId?: number) => {
    try {
      ...
      // Get live_photo URL to exclude it from images
      const livePhotoUrl = data?.live_photo || null;

      // Extract images from various possible fields, excluding live_photo
      let extractedImages: ImageItem[] = [];
      
      if (data?.images && Array.isArray(data.images)) {
        extractedImages = data.images
          .map((img: any) => typeof img === 'string' ? { url: img, image: img } : img)
          .filter((img: ImageItem) => {
            const imgUrl = typeof img === 'string' ? img : (img.url || img.image || img.photo || img.picture);
            return imgUrl && imgUrl !== livePhotoUrl;
          });
        console.log('Found images in data.images:', extractedImages);
      } else if (data?.photos && Array.isArray(data.photos)) {
        extractedImages = data.photos
          .map((img: any) => typeof img === 'string' ? { url: img, image: img } : img)
          .filter((img: ImageItem) => {
            const imgUrl = typeof img === 'string' ? img : (img.url || img.image || img.photo || img.picture);
            return imgUrl && imgUrl !== livePhotoUrl;
      ...
      console.log('=== EXTRACTED IMAGES (excluding live_photo) ===');
      ...
      // Set banner image with priority: banner_image/banner field > first image from images (excluding live_photo) > default
      let bannerUrl: string | null = null;
      
      // Priority 1: Check for dedicated banner field
      if (data?.banner_image) {
        bannerUrl = data.banner_image;
        console.log('Using banner_image field:', bannerUrl);
      } else if (data?.banner) {
        bannerUrl = data.banner;
        console.log('Using banner field:', bannerUrl);
      }
      // Priority 2: Use first image from extracted images (already excluding live_photo)
        ...
        if (bannerUrl) {
          console.log('Using first image as banner:', bannerUrl);
        }
      }
      ...
      if (bannerUrl) {
        ...
        setBannerImage(null);
        console.log('No banner image found, using default background');
      }
            ...
            <View style={styles.profileInfoContainer}>
              <View style={styles.thumbnailContainer}>
                <Image
                  source={
                    (() => {
                      // Priority 1: First image from images array (excluding live_photo)
                      if (images.length > 0) {
                        const firstImage = images[0];
                        const imageUrl = typeof firstImage === 'string' 
                          ? firstImage 
                          : (firstImage.url || firstImage.image || firstImage.photo || firstImage.picture);
                        if (imageUrl) {
                          return { uri: imageUrl };
                        }
                      }
                      // Priority 2: Fallback to profile_photo field
                      if (profileData?.profile_photo || profileData?.profilePicture || profileData?.profile_picture) {
                        return { uri: profileData.profile_photo || profileData.profilePicture || profileData.profile_picture };
                      }
